          ...
          <>
            ...
            <div className="db-banner-row">
              ...
              <div className="db-highlight-col">
                {[
                  { url: 'https://student-fines-hub-vf9z.vercel.app/', cls: 'db-hl-card--blue', id: 'csc', tag: 'CSC', name: 'Student Fines Hub' },
                  { url: 'https://osas-sys.duckdns.org/', cls: 'db-hl-card--red', id: 'osas', tag: 'OSAS', name: 'Violation Tracker' },
                  { url: 'https://ecnesis.duckdns.org/', cls: 'db-hl-card--gold', id: 'admission', tag: 'ADMISSIONS', name: 'ECNESIS Portal' },
                ].map(card => (
                  <a key={card.id} href={card.url} target="_blank" rel="noopener noreferrer"
                    className={'db-hl-card ' + card.cls}
                    onClick={() => { recordVisit(card.id, card.name); setAnalytics(getAnalytics()); }}>
                    <div className="db-hl-body">
                      <p className="db-hl-tag">{card.tag}</p>
                      <p className="db-hl-name">{card.name}</p>
                      <p className="db-hl-visits">{analytics[card.id] || 0} visits</p>
                    </div>
                  </a>
                ))}
              </div>
              ...
              {filtered.length === 0 ? (
                ...
                <div className="db-sys-grid">
                  {filtered.map((sys) => {
                    ...
                    return (
                      <button key={sys.id} className="db-sys-card"
