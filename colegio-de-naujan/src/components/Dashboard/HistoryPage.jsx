...
async function fetchLoginHistory(limit = 200) {
...
async function fetchVisitHistory(limit = 200) {
  const { data, error } = await supabase
    .from("system_visits")
...
const HistoryPage = ({ user }) => {
  ...
  const filteredLogins = loginHistory.filter(row => {
    ...
    const matchC = courseFilter === "all" || (row.course || "").toUpperCase().includes(courseFilter);
  ...
  const filteredVisits = visitHistory.filter(row =>
    !q
    || (row.user_id      || "").toLowerCase().includes(q)
    || (row.system_label || "").toLowerCase().includes(q)
  );
          ...
          <div className="db-panel" style={{ padding:0, overflow:"hidden" }}>
            <div style={{ padding:"14px 18px", borderBottom:"1px solid #f3f4f6", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
              <div>
                ...
                <p style={{ margin:0, fontSize:"0.75rem", color:"#6b7280" }}>
                  {loading ? "Loading…" : `${filteredLogins.length} record${filteredLogins.length !== 1 ? "s" : ""} — name, course, login time, status`}
                </p>
              </div>
              <LiveDot />
            </div>
            ...
            {loading ? (
              ...
              <div>
                {/* Column labels */}
                <div style={{
                  display:"grid", gridTemplateColumns:"2fr 1.3fr 1.4fr 1fr",
                  padding:"8px 18px", background:"#f9fafb", borderBottom:"1px solid #f3f4f6",
                  fontSize:"0.68rem", fontWeight:700, color:"#9ca3af", textTransform:"uppercase", letterSpacing:"0.05em",
                }}>
                ...
                {filteredLogins.map(row => {
                  ...
                  return (
                    <div key={row.id} style={{
                      ...
                      alignItems:"center", background: online ? "#f9fff9" : "#fff",
                    }}>
                      {/* Name + student no. */}
                      <div style={{ display:"flex", flexDirection:"column", gap:2 }}>
                        <span style={{ fontWeight:700, fontSize:"0.85rem", color:"#0F1422" }}>
                      ...
                      <div>
                        {online ? (
                          <span style={{
                            ...
                            background:"#edf7f1", color:"#10813f",
                            borderRadius:20, padding:"3px 8px", fontSize:"0.69rem", fontWeight:700,
                          }}>
                            <span style={{ width:6, height:6, borderRadius:"50%", background:"#10813f", display:"inline-block" }}/>
                            Online
                          </span>
        ...
        {activeTab === "visits" && (
          <div className="db-panel" style={{ padding:0, overflow:"hidden" }}>
            ...
            {loading ? (
              ...
              <div>
                <div style={{
                  display:"grid", gridTemplateColumns:"1.5fr 2fr 1.3fr",
                  padding:"8px 18px", background:"#f9fafb", borderBottom:"1px solid #f3f4f6",
                  fontSize:"0.68rem", fontWeight:700, color:"#9ca3af", textTransform:"uppercase", letterSpacing:"0.05em",
                }}>
                ...
                {filteredVisits.map(row => {
                  ...
                  return (
                    <div key={row.id} style={{
                      ...
                      {/* Student no. avatar + text */}
                      <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                        <div style={{
                          width:30, height:30, borderRadius:8, flexShrink:0,
                          background:"#f3f4f6", display:"flex", alignItems:"center",
                          justifyContent:"center", fontSize:"0.75rem", fontWeight:800, color:"#6b7280",
                        }}>
                          {row.user_id ? row.user_id.charAt(0).toUpperCase() : "?"}
                        </div>
                        <span style={{ fontWeight:700, fontSize:"0.82rem", color:"#0F1422" }}>
                      ...
                      <div>
                        <span style={{
                          ...
                          background: sys.bg || "#f3f4f6", color: sys.color,
